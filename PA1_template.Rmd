...
The following code downloads data on the fly from [here] (`r url_zipped_data`) to the data_dir. The unzipped local copy is available at data_dir/activity.csv. 
```{R results="hide"}
...
```
A quick sanity check on the data set was carried out. `r exploratory_message`  
**`r exploratory_result`**
...
```
The `r max_interval` time interval average is the maximum as represented by the peak in the graph.
...
``` 
There are a total of `r total_NA_values` NA values in the dataset
...
```
The mean and median of the imputed total steps a day are `r mean_imputed_total_steps_given_day` and `r median_imputed_total_steps_given_day` respectively. 
